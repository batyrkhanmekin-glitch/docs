...
OAuth 2.0 integrations are **per-user**: an administrator configures the OAuth application credentials (`client_id` and `client_secret`) once, and each member connects under their own Atlassian account. Calls to Confluence run as the individual member — no shared token.

**Steps:**

2.2.1. Enable the **Use OAuth 2.0 sign-in** toggle in the integration form. The form switches to show the OAuth application fields (`Client ID`, `Client Secret`, `Callback Base URL`). These are filled in by the administrator for shared integrations, or by the user when creating a personal integration.

2.2.2. Click **Sign in with Confluence** (or the OAuth sign-in button that appears). A browser popup opens the Atlassian authorization page.
